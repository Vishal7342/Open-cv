...
#     input = takeCommand()
#     print(input)
#     url = "http://api.brainshop.ai/get?bid=157984&key=3S0hhLXZ5GS2KYs4&uid=[uid]&msg=[{query}]".format(input)
...
class JarvisApp(App):
    def build(self):
        ...
        self.button = Button(text="Start", on_press=self.run_command, size_hint=(1, 0.2))
        layout.add_widget(self.label)
        layout.add_widget(self.button)
        return layout

    def run_command(self, instance):
        # Call the run_alpha function here
        threading.Thread(target=run_alpha, daemon=True).start()
